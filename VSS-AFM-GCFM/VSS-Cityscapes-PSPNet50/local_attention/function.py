...
#import os, sys
#lib_path = os.path.abspath(os.path.join('/media/DATA3/asm/Pycharm-Project/local-attention/build/lib.linux-x86_64-3.8/'))
#sys.path.append(lib_path)

import localAttention
...
class similarFunction(Function):
    ...
    def forward(ctx, x_ori, x_loc, kH, kW):
        ...
        ctx.kHW = (kH, kW)
        output = localAttention.similar_forward(x_ori, x_loc, kH, kW)
    ...
    def backward(ctx, grad_outputs):
        ...
        kH, kW = ctx.kHW
        grad_ori = localAttention.similar_backward(x_loc, grad_outputs, kH, kW, True)
        grad_loc = localAttention.similar_backward(x_ori, grad_outputs, kH, kW, False)
...
class weightingFunction(Function):
    ...
    def forward(ctx, x_ori, x_weight, kH, kW):
        ...
        ctx.kHW = (kH, kW)
        output = localAttention.weighting_forward(x_ori, x_weight, kH, kW)
    ...
    def backward(ctx, grad_outputs):
        ...
        kH, kW = ctx.kHW
        grad_ori = localAttention.weighting_backward_ori(x_weight, grad_outputs, kH, kW)
        grad_weight = localAttention.weighting_backward_weight(x_ori, grad_outputs, kH, kW)
