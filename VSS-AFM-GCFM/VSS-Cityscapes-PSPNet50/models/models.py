...
class SegmentationModule(SegmentationModuleBase):
    ...
    def forward(self, feed_dict, *, segSize=None):
        ...
        if segSize is None:
            ...
            del cur_feature, cur_feature_all, low_cur_feature, pre_1_feature, pre_2_feature, pre_5_feature
...
class Class_Module(nn.Module):
    ...
    def gen_prototypes(self, feat_y, logit_y, feat_x, logit_x):
        ...
        #label_y = torch.argmax(logit_y, dim=-1) # n, h*w
        ...
        #label_x = torch.argmax(logit_x, dim=-1) # n, h*w

        feat = torch.cat((feat_y, feat_x), dim=1) # n, 2*h*w, dim
        #logit = torch.cat((logit_y, logit_x), dim=1) # n, 2*h*w, c
        ...
        for i in range(n):
            ...
            prototypes = prototypes.permute(1, 0).contiguous().unsqueeze(0)
            prototypes_batch.append(prototypes)
        return prototypes_batch
        '''
        '''
        classes = torch.unique(label.clone().detach())
        prototypes = []
        for c in classes:
            prototype = feat[label == c, :]
            prototype = prototype.mean(0, keepdims=True)
        ...
        prototypes = torch.cat(prototypes, dim=0)
        prototypes = prototypes.permute(1, 0).contiguous().unsqueeze(0)
        prototypes_batch = prototypes.repeat(n,1,1).contiguous()
        '''
        #cla = logit.shape[2]
        #label2 = F.one_hot(label, num_classes=cla) # n, 2*h*w, c
        label2 = label
    ...
    def forward(self, x, y):   # x:pre; y:cur
        ...
        #prototypes_y = self.gen_prototypes(y, logit_cur)

        #prototypes_x = self.gen_prototypes(x, logit_pre)

        prototype3 = self.gen_prototypes(y, logit_cur, x, logit_pre)
        b = logit_cur.shape[0]

        #prototype3 = torch.cat((prototypes_x, prototypes_y), dim=2) # (batch, in_dim, 2*class)
...
class Local_Module5(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_all = torch.cat((attn1, attn2), 1)
        attn_all =  attn_all * self.scale
        attn_all = attn_all.permute(0, 2, 3, 1)
        attn = self.softmax(attn_all)
...
class FOCALDeepsup(nn.Module):
    ...
    def forward(self, conv_out, segSize=None):
        ...
        conv5 = torch.cat([conv2, conv1, conv_cur],1)
        batch_size, num_clips, _, h_ori, w_ori = conv5.shape
        conv5 = conv5.reshape(batch_size*num_clips, -1, h_ori, w_ori)
        _c1 = self.conv_fea1(conv5)
        _c = _c1.reshape(batch_size, num_clips, -1, h_ori, w_ori)
        ...
        #c_further = torch.add(x, _c2)
        #c_further = torch.add(c_further, _c3)
        ...
        if self.use_softmax:  # is True during inference
            ...
            x2 = nn.functional.interpolate(x2, size=(713, 713), mode='bilinear', align_corners=True)
            _ = nn.functional.interpolate(_, size=(713, 713), mode='bilinear', align_corners=True)
            class_out = nn.functional.interpolate(class_out, size=(713, 713), mode='bilinear', align_corners=True)
